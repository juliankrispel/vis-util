(function(){
  var visUtil = {
    ...
    createTickObjects: function(data, size, ticks, minGap, isReverse){
        if(!_.isArray(data)){
        ...
        if(isReverse){
            scale = this.createScale(min, max, size, 0);
        }else{
            scale = this.createScale(min, max, 0, size);
        }
